...
export default {
  ...
  created() {
    ...
    axios.get('http://localhost:9090/course/findCouById/' + this.ruleForm.cid).then(function (resp) {
      that.ruleForm = resp.data.content[0]
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.post("http://localhost:9090/course/updateCou", this.ruleForm).then(function (resp) {
