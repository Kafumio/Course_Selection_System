...
export default {
  methods: {
    deleteTeacher(row) {
      ...
      const that = this
      axios.get('http://localhost:9090/teacher/deleteTeaById/' + row.tid).then(function (resp) {
  ...
  watch: {
    ruleForm: {
      handler(newRuleForm, oldRuleForm) {
        ...
        that.total = null
        that.tableData = null
        axios.post("http://localhost:9090/teacher/findTeaBySearch", newRuleForm).then(function (resp) {
          console.log("查询结果:");
