...
export default {
  ...
  created() {
    ...
    axios.get('http://localhost:9090/student/findStuById/' + this.ruleForm.sid).then(function (resp) {
      that.ruleForm = resp.data.content
    })
  },
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.post("http://localhost:9090/student/updateStu", this.ruleForm).then(function (resp) {
