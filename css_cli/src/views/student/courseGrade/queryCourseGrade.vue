...
export default {
  ...
  created() {
    const that = this
    axios.get('http://localhost:9090/SCT/findAllTerm').then(function (resp) {
      that.termList = resp.data.content
    })
  },
  watch: {
    term: {
      handler(newTerm, oldTerm) {
        ...
        const that = this
        axios.get('http://localhost:9090/SCT/findCTInfoBySid/' + sid + '/' + newTerm).then(function (resp) {
