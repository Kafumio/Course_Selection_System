    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.get('http://localhost:9090/info/getCurrentTerm').then(function (resp) {
            sessionStorage.setItem("currentTerm", resp.data.content)
          })

          axios.get('http://localhost:9090/info/getForbidCourseSelection').then(function (resp) {
            sessionStorage.setItem("ForbidCourseSelection", resp.data.content)
          })

          if (that.ruleForm.type === 'admin' || that.ruleForm.type === 'teacher') {
            let form = {tid: that.ruleForm.id, password: that.ruleForm.password}
            console.log(form)
            axios.post("http://localhost:9090/teacher/loginTea", form).then(function (resp) {
              console.log("教师登陆验证信息：" + resp.data.content)
              check = resp.data.info
              if (check === true) {
                axios.get("http://localhost:9090/teacher/findTeaById/" + that.ruleForm.id).then(function (resp) {
                  console.log("登陆页正在获取用户信息" + resp.data)
                  name = resp.data.content.tname

                  sessionStorage.setItem("token", 'true')
                  sessionStorage.setItem("type", that.ruleForm.type)
                  sessionStorage.setItem("name", name)
                  sessionStorage.setItem("tid", resp.data.content.tid)
            ...
            axios.post("http://localhost:9090/student/loginStu", form).then(function (resp) {
              console.log("学生登陆验证信息：" + resp.data.content)
              check = resp.data.info
              if (check === true) {
                axios.get("http://localhost:9090/student/findStuById/" + that.ruleForm.id).then(function (resp) {
                  console.log("登陆页正在获取用户信息" + resp.data.content)
                  name = resp.data.content.sname

                  sessionStorage.setItem("token", 'true')
                  sessionStorage.setItem("type", that.ruleForm.type)
                  sessionStorage.setItem("name", name)
                  sessionStorage.setItem("sid", resp.data.content.sid)
